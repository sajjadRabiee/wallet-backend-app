...
func Test_userService_GetUser(t *testing.T) {
	...
	t.Run("test success get user", func(t *testing.T) {
		userRepository.Mock.On("FindById", 1).Return(&model.User{ID: 1, Name: "nabil", PhoneNumber: "+989123456789", Password: "12345"}, nil).Once()
		...
		assert.Equal(t, "+989123456789", user.PhoneNumber)
	})
...
func Test_userService_CreateUser(t *testing.T) {
	...
	t.Run("test success create user", func(t *testing.T) {
		userRepository.Mock.On("FindByPhoneNumber", "+989123456789").Return(&model.User{}, nil).Once()
		userRepository.Mock.On("Save", mock.Anything).Return(&model.User{ID: 1, Name: "nabil", PhoneNumber: "+989123456789", Password: "$2a$04$Zw.U9pmOuDXif7bQ2hwVBOWkV3HOMDM6sdzetwtTrucoYWseqtYje"}, nil).Once()

		input := &dto.RegisterRequestBody{}
		input.Name = "nabil"
		input.PhoneNumber = "+989123456789"
	...
	t.Run("test error phone_number not valid when create user", func(t *testing.T) {
		input := &dto.RegisterRequestBody{}
		input.Name = "nabil"
		input.PhoneNumber = "+989123456789"
		input.Password = "12345"
		...
		assert.NotNil(t, err)
		assert.Equal(t, &custom_error.NotValidPhoneNumberError{}, err)
		assert.Equal(t, uint(0), user.ID)
		assert.Equal(t, "", user.Name)
		assert.Equal(t, "", user.PhoneNumber)
	})
	...
	t.Run("test error db find user when create user", func(t *testing.T) {
		userRepository.Mock.On("FindByPhoneNumber", "+989123456789").Return(&model.User{}, errors.New("something went wrong")).Once()
	...
	t.Run("test error user already exists when create user", func(t *testing.T) {
		userRepository.Mock.On("FindByPhoneNumber", "+989123456789").Return(&model.User{ID: 1, Name: "nabil", PhoneNumber: "+989123456789", Password: "12345"}, nil).Once()
	...
	t.Run("test error db save user when create user", func(t *testing.T) {
		userRepository.Mock.On("FindByPhoneNumber", "+989123456789").Return(&model.User{}, nil).Once()
		userRepository.Mock.On("Save", mock.Anything).Return(&model.User{ID: 0, Name: "nabil", PhoneNumber: "+989123456789", Password: "$2a$04$Zw.U9pmOuDXif7bQ2hwVBOWkV3HOMDM6sdzetwtTrucoYWseqtYje"}, errors.New("something went wrong")).Once()
		...
		assert.Equal(t, uint(0), user.ID)
		assert.Equal(t, "nabil", user.Name)
		assert.Equal(t, "+98123456789", user.PhoneNumber)
	})
}
